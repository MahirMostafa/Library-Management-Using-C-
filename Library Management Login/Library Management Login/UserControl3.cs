...
namespace Library_Management_Login
{
    internal class UserControl3
    {
        ...
        public string create(string AIUBId, string Name, string password, string mail, string Phonenumber, string gender, string address, string birthdate, string UserType)
        {
            ...
            try
            {
                com = new SqlCommand("INSERT INTO LibrarianSignUp (AIUBId,Name,password,mail,Phonenumber,gender,address,birthdate,UserType) values (@AIUBId,@Name,@password,@mail,@Phonenumber,@gender,@address,@birthdate,@UserType)", con);
